<h1 align="center">Hi 👋, I'm Vinay Datta Mucharla</h1>
<h3 align="center">Aspiring VLSI Design & Verification Engineer</h3>


![](https://komarev.com/ghpvc/?username=vinaydatta416-github-username&base=2536)

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=vinaydatta416-github-username&base=2536&color=brightgreen&style=for-the-badge" alt="Profile views"/>
</p>

---

## 📝 Summary
Detail-oriented and motivated **VLSI Design & Verification Engineer** with hands-on experience in **RTL design, testbench development, and protocol verification**. Proficient in **Verilog, SystemVerilog, UVM**, and **AMBA protocols (APB, AHB)**. Skilled in building reusable verification environments, developing assertions, and achieving high coverage metrics. Eager to contribute to **cutting-edge semiconductor design teams**.  

---

## ⚡ VLSI Domain Skills & Tools

<p align="left">
  <!-- HDL -->
  <img src="https://img.shields.io/badge/Verilog-8A2BE2?style=for-the-badge&logo=verilog&logoColor=white" />
  <img src="https://img.shields.io/badge/SystemVerilog-FF1493?style=for-the-badge&logo=verilog&logoColor=white" />
  <img src="https://img.shields.io/badge/UVM-000000?style=for-the-badge&logo=uml&logoColor=white" />
  
  <!-- Scripting -->
  <img src="https://img.shields.io/badge/Linux-000000?style=for-the-badge&logo=linux&logoColor=white" />
  <img src="https://img.shields.io/badge/Shell_Scripting-FFD700?style=for-the-badge&logo=gnu-bash&logoColor=black" />
  <img src="https://img.shields.io/badge/Perl-39457E?style=for-the-badge&logo=perl&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/C_Programming-00599C?style=for-the-badge&logo=c&logoColor=white" />

  <!-- Tools -->
  <img src="https://img.shields.io/badge/Synopsys%20VCS-FF4500?style=for-the-badge&logo=synopsys&logoColor=white" />
  <img src="https://img.shields.io/badge/QuestaSim-4682B4?style=for-the-badge&logo=mentor&logoColor=white" />
  <img src="https://img.shields.io/badge/Xilinx%20Vivado-AA0000?style=for-the-badge&logo=xilinx&logoColor=white" />
  <img src="https://img.shields.io/badge/EDA_Playground-008080?style=for-the-badge&logo=verilog&logoColor=white" />
  <img src="https://img.shields.io/badge/VSCode-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</p>

---

## 💻 Projects

- 🔹 **AHB2APB Bridge Verification using UVM **  
  - Designed and verified a bridge to convert AHB transactions to APB operations.  
  - Developed a reusable UVM-based testbench for transaction flow and protocol checks.  
  - Implemented **SystemVerilog Assertions (SVA)** to validate timing and conversions.  
  - Achieved **100% functional & code coverage**.  

- 🔹 **Dual-Port RAM Design & Verification**  
  - Designed a Dual-Port RAM with simultaneous read/write support.  
  - Verified memory behavior using UVM, with directed & constrained-random tests.  
  - Validated corner cases like simultaneous access to same address.  
  - Used **functional coverage & assertions** for data integrity.  

- 🔹 **1x3 Router Design & Verification**  
  - Developed a **1x3 packet-based router** with header-based routing.  
  - Built a **UVM environment** (driver, monitor, scoreboard, coverage).  
  - Verified valid/invalid headers and congestion handling.  
  - Achieved **100% functional coverage**.  

---

## 💼 Work Experience
- **Maven Silicon** – *RTL Design and Verification Trainee* *(Aug 2024 – Apr 2025)*  
  - Hands-on with **Verilog, SystemVerilog, UVM, Assertions**.  
  - Built multiple **UVM-based testbenches** and executed full verification flow for IPs.  

---

## 📜 Certificate
- **Verilog, SystemVerilog, and UVM Certification – Maven Silicon**  

---

## 🔗 EDA Links
- 🌐 **LinkedIn**: [Vinay Datta Mucharla](https://www.linkedin.com/in/vinaydattamucharla/)  
- 💻 **GitHub**: [vinaydatta416](https://github.com/vinaydatta416)  
- ▶️ **EDA Playground**: [Vinay Datta Mucharla](https://www.edaplayground.com)  

---

## 📊 GitHub Stats
<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=vinaydatta416&show_icons=true&theme=dracula&count_private=true" height="170" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=vinaydatta416&layout=compact&langs_count=5&theme=dracula" height="170" />
</div>

---

## 🔥 Streak Stats
---

![Snake animation](https://github.com/CruzNadin/cruznadin/raw/main/github-contribution-grid-snake.gif)

![T-Rex Game](https://github.com/knickers/chrome-t-rex-game/raw/master/t-rex.gif)

![Circuit Animation](https://i.makeagif.com/media/2-02-2023/BboWfJ.gif)
